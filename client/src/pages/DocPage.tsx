...
// Legacy sample docs for fallback (can be removed once all docs are in markdown)
...
export default function DocPage() {
  ...
  const loadMarkdownFile = async (path: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        // Fall back to sample docs
        const fallbackDoc = sampleDocs[fullPath] || sampleDocs["/docs/getting-started"];
        setDocData({
          title: fallbackDoc.title,
          content: fallbackDoc.content,
          breadcrumbs: fallbackDoc.breadcrumbs,
        });
        setLoading(false);
        return;
      }
      ...
      setDocData({
        ...
        breadcrumbs: generateBreadcrumbs(fullPath),
        metadata,
      });
    } catch (error) {
      console.error("Error loading markdown:", error);
      // Fall back to sample docs on error
      const fallbackDoc = sampleDocs[fullPath] || sampleDocs["/docs/getting-started"];
      setDocData({
        title: fallbackDoc.title,
        content: fallbackDoc.content,
